...
OUTPUT_DIR = os.getenv('LEX_OUTPUT_DIR', '/trading/data/lex_ladders/')
...
def lex_ladder_to_csv(data_list, filename):
    if not data_list:
    ...
    for i, dictionary in enumerate(data_list):
        if dictionary.keys() != headers:
    ...
    print(f"CSV file '{filename}' has been created successfully with {len(data_list)} rows")


def build_calendars(sec_list, days_back):
    cals = [] 
    for sec in sec_list.split(','):
        cals.append(show_calendar(sec, days_back))

    html_tables = '<br><br>'.join(cals)

    subject  = f'Multi LEX Monitor: {datetime.today():%Y-%m-%d}'

    with open(f'{OUTPUT_DIR}/lex_calendars.html', "w") as lex_cals:
            lex_cals.write(html_tables)

    for recipient in ['xjcarter@gmail.com', 'gerome.gregory@yahoo.com', 'sjsarce@gmail.com']:
        email_lib.send_html(recipient, subject, html_tables)
...
def show_calendar(stock, days_back):
    ...
    ## dump the lex table to csv
    lex_ladder_to_csv(lex_list, f'{OUTPUT_DIR}/{stock}_lex.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sec_list", required=True, help="securities to track")
    parser.add_argument("--history", type=int, help="days back fo history", default=20)
    u = parser.parse_args()

    build_calendars(u.sec_list, u.history)
