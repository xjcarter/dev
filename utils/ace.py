...
PORTFOLIO_DIRECTORY = os.environ.get('PORTFOLIO_DIRECTORY', '/portfolio/')
...
## updated sub_account parsing
def get_available_equity(sub_account):
    try:
        summary = IB.get_account_summary(sub_account)
        """
        grabbing info from the key in the summary:
        ...
        "availablefunds": {
            "amount": 1068573.25,
            "currency": "USD",
            "isNull": false,
            "timestamp": 1773187711000,
            "value": null,
            "severity": 0
        },
        ...
        """
        ## Note - assuming USD...
        return float(summary['availablefunds']['amount'])
    except:
        logger.critical(f'could not retrieve available fund for account: {sub_account}')
...
## 2026-02-10:
## after consulting with IB - this is the solution we came up
## with to get consistent data both in PROD and PAPER
def get_sub_account_info():
    accounts = IB.get_accounts()
    all_accounts = set(accounts['accounts'])
    profiles = set(accounts['profiles'])

    ## all REAL subaccounts handle cash accounts -
    ## and do not show up in 'profiles'
    sub_accounts = all_accounts ^ profiles
...
def fetch_and_post( config_mask, strategy_list ):
    ...
    for name, cash in account_cash.items():
        ...
        report_data_dict[name] = u

    account_file =f"{PORTFOLIO_DIRECTORY}/{strategy_id}/account/{strategy_id}.alloc.{get_time()}.json"
    with open(account_file, 'w') as f:
        acc_info = json.dumps(report_data_dict, ensure_ascii=False, indent=4)
        f.write(acc_info + '\n')
...
def run(alloc_config, report, strategies=None):
    ...
    ## Do IBKR connection setup
    IB.establish_connection()

    ## IMPORTANT needed for initial setups to grab account info AND market data
    IB.get_accounts()
