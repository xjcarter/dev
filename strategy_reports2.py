...
class Reporter:
    ...
    def generate_metrics(self) -> bool:
        ...
        # ── guard: equity curve must not start at zero ─────────────
        ...
        # ── win / loss averages and dispersion ─────────────────────
        avg_win = trade_wins.mean() if len(trade_wins) else 0.0
        avg_loss = trade_losses.mean() if len(trade_losses) else 0.0
        std_win = trade_wins.std() if len(trade_wins) > 1 else 0.0
        std_loss = trade_losses.std() if len(trade_losses) > 1 else 0.0

        avg_dollar_win = dollar_wins.mean() if len(dollar_wins) else 0.0
        avg_dollar_loss = dollar_losses.mean() if len(dollar_losses) else 0.0

        # ── profit factor (dollar-based is more accurate than %) ───
        gross_profit = dollar_wins.sum() if len(dollar_wins) else 0.0
        gross_loss = abs(dollar_losses.sum()) if len(dollar_losses) else 0.0
        ...
        # average drawdown — mean of all underwater readings
        drops = drawdown_series[drawdown_series < 0]
        avg_dd = drops.mean() if len(drops) else 0.0

        # max drawdown duration (in trading days)
        underwater = (drawdown_series < 0).astype(int)
        # group consecutive underwater days and find the longest run
        recovery_points = underwater.diff().fillna(0)
        dd_groups = recovery_points.ne(0).cumsum()
        ...
        expectancy = dollar_pnl.mean()

        # ── payoff ratio: |avg win %| / |avg loss %| ──────────────
        payoff = abs(avg_win / avg_loss) if avg_loss != 0 else float('inf')

        # ── Kelly criterion: W - (1 - W) / payoff ─────────────────
        # represents theoretically optimal fraction of capital to risk
        kelly = win_pct - ((1 - win_pct) / payoff) if payoff != 0 and payoff != float('inf') else 0.0

        # ── max consecutive wins / losses ──────────────────────────
        win_flags = (dollar_pnl > 0).astype(int)
        streak_groups = win_flags.diff().ne(0).cumsum()
        streak_sizes = win_flags.groupby(streak_groups).agg(['sum', 'count'])
        # winning streaks: groups where every trade won (sum == count)
        win_streaks = streak_sizes[streak_sizes['sum'] == streak_sizes['count']]
        loss_streaks = streak_sizes[streak_sizes['sum'] == 0]
        max_consec_wins = int(win_streaks['count'].max()) if len(win_streaks) else 0
        ...
        avg_trade_dd = trades_df['Drawdown'].mean()
        max_trade_dd = trades_df['Drawdown'].max()

        # ── net profit ─────────────────────────────────────────────
        net_profit = dollar_pnl.sum()
        ...
        self.metrics = dict(
            ...
            AvgDollarWin=avg_dollar_win,
            AvgDollarLoss=avg_dollar_loss,
