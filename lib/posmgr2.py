...
class PosNode:
    ...
    def clear(self) -> None:
        """Reset all position data to initial state."""
        self.position = 0
        self.duration = 0
        self.price = 0.0
        self.high = None
    ...
    def update_high(self, high: Optional[float]) -> None:
        """Update the high price if the new value is higher."""
        if high is not None and high > 0:
            self.high = max(high, self.high or high)

    def update_low(self, low: Optional[float]) -> None:
        """Update the low price if the new value is lower."""
    ...
    def stamp_with_time(self) -> None:
        """Set timestamp to current time."""
        now = datetime.now()
        self.timestamp = now.strftime("%Y%m%d-%H:%M:%S")

    def to_dict(self) -> Dict[str, Any]:
        """Convert position data to dictionary for JSON serialization."""
        return self.__dict__.copy()

    def from_dict(self, json_dict: Dict[str, Any]) -> 'PosNode':
        """
        Load data from dictionary (in-place update).
        COMPATIBILITY: Returns self for legacy method chaining.
        """
    ...
    def to_json(self) -> str:
        """Serialize to JSON string for logging/persistence."""
        return json.dumps(self.to_dict(), indent=2)

    @classmethod
    def from_json(cls, json_str: str) -> 'PosNode':
        """Deserialize from JSON string."""
        data = json.loads(json_str)
        return cls.from_dict(data)

    def copy(self) -> 'PosNode':
    ...
    def __eq__(self, other: object) -> bool:
        """Check equality of two PosNode instances."""
...
class PosMgr():
    ...
    def check_orders(self):
        ...
        for f in fill_package['fills']:
            ...
            try:
                ...
                nw = f.copy()
                nw['order_notes'] = order.order_notes
    ...
    def register_order(self, order_info):
        ...
        #optionals
        order.order_notes = order_info.get('order_notes')
        ...
        logger.info(
            f'order {order_id}: symbol={order.symbol}, side={order.side}, qty={order.qty}'
        )
